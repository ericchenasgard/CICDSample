...
namespace Net_Core_Common_API.Controllers.User
{
    ...
    public class UserInfoController : ControllerBase
    {
        ...
        /// <param name="oIUserInfo">選單作業</param>
        public UserInfoController(IDoNLog oIDoNLog, IDoJson oIDoJson, ISetResponse oISetResponse, IGetUserInfo oIGetUserInfo, IUserInfo oIUserInfo)
        {
        ...
        [HttpPost("Create")]
        public ActionResult Create(CIn_UserInfo_PageData oCIn_UserInfo_PageData)
        {
            // 取得執行結果(回傳資料/成功筆數)
            var (oCTab_UserInfo, iChange, sMessage) = this._oIUserInfo.fnInsert(oCIn_UserInfo_PageData, this._oIGetUserInfo.fnGetUserInfo());

            #region 回傳資料/訊息/狀態
            // 回傳資料
            string sOut_Data = this._oIDoJson.fnDoSerializeObject(oCTab_UserInfo);
        ...
        [HttpPost("Update")]
        public ActionResult Update(CIn_UserInfo_Update oCIn_UserInfo_Update)
        {
            // 取得執行結果(回傳資料/成功筆數)
            var (oCTab_UserInfo, iChange, sMessage) = this._oIUserInfo.fnUpdate(oCIn_UserInfo_Update, this._oIGetUserInfo.fnGetUserInfo());
        ...
        {
            // 取得執行結果(成功筆數)
            var (iChange, sMessage) = this._oIUserInfo.fnDelete(lPk);
        ...
        [HttpPost("Get")]
        public ActionResult Get([FromQuery] CIn_UserInfo_Search oCIn_UserInfo_Search)
        {
            // 取得執行結果(回傳資料)
            var (lCTab_UserInfo, sMessage) = this._oIUserInfo.fnGet(oCIn_UserInfo_Search);

            #region 回傳資料/訊息/狀態
            // 回傳資料
            string sOut_Data = this._oIDoJson.fnDoSerializeObject(lCTab_UserInfo);
